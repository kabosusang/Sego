...
GameObjectInstance::GameObjectInstance(std::string name)
{
    set_name(name);
}
...
Component *GameObjectInstance::AddComponent(std::string component_type_name)
{
...
std::vector<Component*> GameObjectInstance::GetComponents(std::string component_type_name)
{
   return component_type_instance_map_[component_type_name];
}

Component* GameObjectInstance::GetComponent(std::string component_type_name) {
    if(component_type_instance_map_.find(component_type_name)==component_type_instance_map_.end()){
        return nullptr;
    }
