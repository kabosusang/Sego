...
void MeshRenderer::SetMaterial(Material* material) {
    ...
    Vk_Pipeline_.InputShaderPath(material_->vshader_path_,material_->fshader_path_);
    Vk_Pipeline_.CreateGraphicsPipeline();
...
for(int i = 0; i < Vk_Pipeline_.GetPipeLineCount(); i++)
{
 //绑定图形管道
vkCmdBindPipeline(commandBuffer,VK_PIPELINE_BIND_POINT_GRAPHICS,Vk_Pipeline_.GetGraphicsPipeline()[i]);

// 绑定描述符集
    vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, 
    Vk_Pipeline_.GetPipelineLayout()[i], 0, 1, &Obj_DescriptorSets_[app_device->currentFrame] ,0, nullptr);
}
// 绑定索引缓冲区并绘制模型
